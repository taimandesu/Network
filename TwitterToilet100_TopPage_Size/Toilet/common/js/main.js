...
function createPoo()
{
  ...
  // //$popup.append(searchResults[searchCount-1][1]["text"]);
    ...
    $popup.append(text);
    //以下ウンチの大きさ
    textLength = text.length;//取得したtextの文字数
    imgSize = textLength * 1;//ツイートの長さによって大きさを変更
    TweenMax.to($pooimg, 0.1, {width:imgSize,height:imgSize * 0.8});//アニメーションでサイズを変更
